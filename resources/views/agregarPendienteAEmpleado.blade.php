...
<body>
    ...
    <div class="p-4 sm:ml-64 mt-16 md:mt-10">
        ...
        <input type="hidden"  value="{{$pkEmpleado}}" name="pkEmpleado">
                    ...
                    <td>{{ $empleado->nombreUsuario }}</td>
							<td class="items-center flex justify-center">									
								<div class="mt-2 md:mt-5">
									<input type="radio" name="persona-seleccionado" class="seleccionar-persona" data-persona-id="{{$dato->pkPersona}}" class="w-6 h-6 rounded text-green-400 bg-gray-100 border-gray-300 focus:ring-green-400 focus:ring-2">
                                </div>
							</td>
						</tr>
                    @endforeach
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#tablaPersonas tbody').on('change', '.seleccionar-persona', function() {
            var radio = $(this);
            var row = radio.closest('tr');
            var data = tablePersona.row(row).data();
            var personaId = data[0];

            tablePersonaSeleccionados.clear().draw(); // Limpiar la tabla de seleccionados
