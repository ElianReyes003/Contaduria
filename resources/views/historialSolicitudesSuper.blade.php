...
<body>
    <div>
        <div>Historial de Solicitudes super</div>    
    ...
    <label for="estatusPatron" class="block mb-2 text-sm font-medium text-gray-900">Tipo Pendiente</label>
                        <select name="fkTipoSolicitud"  id="fkTipoSolicitud" class="bg-green-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-400 focus:border-green-400 block w-full p-2.5">
                        @php
                        use App\Models\tipoSolicitudSuper;
                        $datosTipoPendiente = tipoSolicitudSuper::get();
                        @endphp
                        <option value="">Seleccion un tipo pendiente</option>
                            @foreach (  $datosTipoPendiente as $dato)
                                <option value="{{$dato->nombreTipoSolicitudSuper}}">{{$dato->nombreTipoSolicitudSuper}}</option>
                            @endforeach
				</select>
                ...
                <label for="estatusPatron" class="block mb-2 text-sm font-medium text-gray-900">Tipo Pendiente</label>
                        <select name="estatusSolicitud"  id="estatusSolicitud" class="bg-green-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-400 focus:border-green-400 block w-full p-2.5">
    ...
    <div class="bg-white rounded-lg shadow-lg mt-10">
        ...
        <table class="w-full table-auto mt-[1rem]" id="tablaPeticiones" class="display nowrap" width="90%">
            <thead class="text-center">
                <tr class="h-24 text-center">
                    <th>Titulo Peticion</th>
                    <th>Nombre Empleado</th>
                    <th>Nombre Completo Cliente o Compañia</th>
                    <th>Categoria Peticion</th>
            ...
            <tbody>
                @foreach ( $todosPendientes as $dato)
                    <tr class="h-20">
                        <td >{{ $dato->tituloSolicitud }}</td>
                        <td >{{ $dato->nombre_empleado.' '.$dato->apellidoPaterno_empleado.' '.$dato->apellidoMaterno_empleado }}</td>
                        <th>
                            @if($dato->nombre_cliente || $dato->apellidoPaterno_cliente || $dato->apellidoMaterno_cliente)
                                {{ $dato->nombre_cliente . ' ' . $dato->apellidoPaterno_cliente . ' ' . $dato->apellidoMaterno_cliente }}
                            @else
                                {{ $dato->nombreComercial }}
                            @endif
                        </th>

                        <th>{{$dato->nombreTipoSolicitudSuper}}</th>
                        <th>{{$dato->fechaSolicitudSuper}}</th>
    ...
    $(document).ready(function() {
        var tablePeticiones = $('#tablaPeticiones').DataTable({
			responsive: true,
            "language": {
    ...
    $('#previousBtn2').on('click', function(e) {
      e.preventDefault();
      tablePeticiones.page('previous').draw(false);
    });

    // Agrega evento de clic al botón Next
    $('#nextBtn2').on('click', function(e) {
      e.preventDefault();
      tablePeticiones.page('next').draw(false);
    });
 

$('#fkTipoSolicitud, #dia,#estatusSolicitud, #mes').change(function () {
        var tipoSolicitud = $('#fkTipoSolicitud').val();
        var filtroMes = $('#mes').val();
        var filtroFecha = $('#dia').val();
        var estatus = $('#estatusSolicitud').val();
        
        // Limpiar todos los filtros antes de aplicar nuevos
        tablePeticiones.columns().search('').draw();

        // Aplicar los nuevos filtros
        if (tipoSolicitud !== "") {
            tablePeticiones.column(3).search(tipoSolicitud).draw(); // Filtro para Nombre de Articulo
        }

        if (estatus !== "") {
            tablePeticiones.column(5).search(estatus).draw(); // Filtro para Nombre de Articulo
        }
       
        if (filtroMes !== "") {
            tablePeticiones.column(4).search(filtroMes).draw(); // Filtro para Mes
        }
        else if (filtroFecha !== "") {
            var fechaFormateada = moment(filtroFecha).format("YYYY-MM-DD");
            tablePeticiones.column(4).search(fechaFormateada).draw(); // Filtro para Fecha
        }
        else {
            // Si no hay filtro de mes ni de fecha, limpia el filtro en esa columna
            tablePeticiones.column(4).search('').draw();
        }
    ...
    $('#busqueda2').on('keyup', function (e) {
        ...
        tablePeticiones.search(filtroBusqueda).draw();
    });

    $('#limpiarFiltros').on('click', function () {
        $('#fkTipoSolicitud, #dia,#estatusSolicitud, #mes').val('');
        tablePeticiones.search('').columns().search('').draw();
    });
