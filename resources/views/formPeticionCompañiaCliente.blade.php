...
<body>
    <div>
        <div>Selecciona  Cliente moral o cliente fisico para peticion {{ $datoEmpleado->nombre. ' '. $datoEmpleado->apellidoPaterno . ' ' . $datoEmpleado->apellidoMaterno  }}</div>    
    </div>

    

    <form id="formulario" action="{{ route('solicitud.insertar') }}" enctype="multipart/form-data" method="POST">
    @csrf
    
    <input type="hidden" name="pkEmpleado" value="{{$datoEmpleado->pkEmpleado}}">
    <label for="estatusPatron" class="block mb-2 text-sm font-medium text-gray-900">Titulo Solicitud</label>
    <input type="text" name="tituloSolicitud">
    


    <label for="estatusPatron" class="block mb-2 text-sm font-medium text-gray-900">Tipo Pendiente</label>
                        <select name="fkTipoSolicitudSuper"  class="bg-green-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-400 focus:border-green-400 block w-full p-2.5">
                        @php
                        ...
                        <option value="">Seleccion un tipo pendiente</option>
                            @foreach (  $datosTipoPendiente as $dato)
                                <option value="{{$dato->pkTipoSolicitudSuper}}">{{$dato->nombreTipoSolicitudSuper}}</option>
                            @endforeach
						</select>
