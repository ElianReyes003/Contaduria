...
<body>
    <div>
        <div>Lista Personal de Empleado  
        {{ $datoEmpleado->nombre.' '.$datoEmpleado->apellidoPaterno .' '. $datoEmpleado->apellidoMaterno }}    
        </div>
</div>
...
<div class="bg-white rounded-lg shadow-lg mt-10">
            ...
            <tr class="h-24 text-center">
                ...
                @foreach ($todosPendientes as $dato ) 
                                            ...
                                            <th>{{$dato->fechaPendienteEmpleado}}</th>
                                            <th><a href="">Confirmar</a></th>
...
$('#fkTipoPendiente, #dia, #mes').change(function () {
        ...
        if (tipoPendiente !== "") {
            tablePendientes.column(1).search(tipoPendiente).draw(); // Filtro para Nombre de Articulo
        }
       
        if (filtroMes !== "") {
            tablePendientes.column(2).search(filtroMes).draw(); // Filtro para Mes
        }
        else if (filtroFecha !== "") {
            var fechaFormateada = moment(filtroFecha).format("YYYY-MM-DD");
            tablePendientes.column(2).search(fechaFormateada).draw(); // Filtro para Fecha
        }
        else {
            // Si no hay filtro de mes ni de fecha, limpia el filtro en esa columna
            tablePendientes.column(2).search('').draw();
        }
